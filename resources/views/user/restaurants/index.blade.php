<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center px-8 py-4 space-x-6 w-full">
            ...
            <form method="GET" action="{{ route('user.restaurants.index') }}" class="flex-grow max-w-xl">
            ...
            <a href="{{ route('user.restaurants.index', ['featured' => true]) }}"
            ...
            @forelse($restaurants as $restaurant)
                <div class="bg-white p-4 rounded-xl shadow hover:shadow-xl transition-transform transform hover:scale-105 relative">
                    ...
                    <a href="{{ route('user.restaurants.show', $restaurant->id) }}"
                     ...
                     <a href="{{ route('restaurant.reviews.index', $restaurant->id) }}"
                        class="text-sm text-green-600 hover:underline mt-1 inline-block">
                        Read Reviews
                    </a>
                </div>
            @empty
                <p class="text-gray-600 text-center col-span-full py-12">No restaurants found. Please try again later!</p>
            @endforelse
        </div>
