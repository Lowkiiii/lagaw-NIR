...
<x-app-layout>
    ...
    <div class="py-8 max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
            @if($spot->img_url)
                <div class="relative">
                    <img src="{{ asset('storage/' . $spot->img_url) }}" alt="{{ $spot->name }}" class="w-full h-60 object-cover">
                    {{-- <a href="{{ route('itineraries.create', $spot->id) }}"
                       class="absolute top-3 right-3 bg-pink-500 text-white px-3 py-1 text-sm rounded-full shadow hover:bg-pink-600 transition">
                        ➕ Add to Itinerary
                    </a> --}}
                </div>
            @endif
        ...
        <div class="bg-white rounded-xl shadow p-6">
            ...
            <form method="POST" action="{{ route('reviews.store', $spot->id) }}" x-data="{ rating: 0 }">
                @csrf
