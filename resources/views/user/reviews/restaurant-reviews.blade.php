...
<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('user.restaurants.index') }}"
    ...
    <div class="py-8 max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
            @if($restaurant->img_url)
                <div class="relative">
                    <img src="{{ $restaurant->img_url }}" alt="{{ $restaurant->name }}" class="w-full h-60 object-cover">
                </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl shadow p-6">
            ...
            <form method="POST" action="{{ route('restaurant.reviews.store', $restaurant->id) }}" x-data="{ rating: 0 }">
                @csrf
