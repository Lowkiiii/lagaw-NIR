...
<x-app-layout>
    ...
    <div class="py-8 max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-xl shadow overflow-hidden">
            @if($cafe->img_url)
                <div class="relative">
                    <img src="{{ $cafe->img_url }}" alt="{{ $cafe->name }}" class="w-full h-60 object-cover">
                </div>
            @endif
        </div>
        ...
        <div class="bg-white rounded-xl shadow p-6">
            ...
            <form method="POST" action="{{ route('cafe.reviews.store', $cafe->id) }}" x-data="{ rating: 0 }">
                @csrf
